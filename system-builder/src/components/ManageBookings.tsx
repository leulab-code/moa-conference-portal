      ...
      <div className="flex overflow-x-auto pb-2 mb-6 border-b border-slate-200 hide-scrollbar">
        {(['payment', 'pending', 'confirmed', 'rejected', 'all'] as TabFilter[]).map((tab) => (
          <button
            ...
            className={`whitespace-nowrap px-6 py-3 text-sm font-bold border-b-2 transition-all ${
              activeTab === tab ? 'border-[#268053] text-[#268053]' : 'border-transparent text-slate-400 hover:text-slate-700 hover:border-slate-300'
            }`}
          >
            ...
            <span className={`ml-2 py-0.5 px-2.5 rounded-full text-xs transition-colors ${
              activeTab === tab ? 'bg-[#268053] text-white' : 'bg-slate-100 text-slate-500'
            }`}>
              {counts[tab]}
            </span>
          </button>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          {serviceNames.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                              {serviceNames.map((name, idx) => (
                                <span key={idx} className="px-2.5 py-1 rounded-md bg-white border border-slate-200 text-slate-700 text-[11px] font-bold shadow-sm">{name}</span>
                              ))}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="space-y-6">
                      <div className="bg-[#268053] border border-[#1a5a3a] rounded-2xl p-6 shadow-lg relative overflow-hidden text-white">
                         ...
                         <p className="text-4xl font-black relative z-10 mb-4">ETB {Number(grandTotal).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
